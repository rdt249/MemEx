...
def comms_off() :
    ...
    for p in pin_list :
        with dio.DigitalInOut(p) as pin :
            ...
            pin.value = False
    '''
    with dio.DigitalInOut(board.D23) as oe :
...
def checkup(pin,status=0x41,show=True) : # test the responsiveness of one DUT, and report the status
    with dio.DigitalInOut(pin) as cs :
        cs.direction = dio.Direction.OUTPUT
        cs.value = True
        ...
        with busio.SPI(board.SCLK,MOSI=board.MOSI,MISO=board.MISO) as spi :
            ...
            if miso[3] == 0x55 :
                ...
                if miso[4] == 0x55 :
                    ...
                    addr = 'addr=unknown'
            if show :
                print(status,mode,hold,rw,addr,sep='\t')

    return result,addr_bytes

def rollcall_old(status=0x41,show=True): # identify responsive memory slots, and report the status
    set_nominal()
    pins = [board.D5,board.D6,board.D13,board.D19,board.D26,board.D24,board.D25,board.D7,board.D12,board.D16,board.D20,board.D21]
    slots = ['A1','A2','A3','B1','B2','B3','C1','C2','C3','D1','D2','D3']
    awake = [False] * len(pins)
    addr_bytes = [False] * len(pins)
    for i in range(len(pins)) :
        if show :
            print(slots[i] + ':',end='\t')
        awake[i],addr_bytes[i] = checkup(pins[i],show=show)
        time.sleep(0.01)
    slots = [x for (x, y) in zip(slots, awake) if y]
    return slots,awake,addr_bytes
